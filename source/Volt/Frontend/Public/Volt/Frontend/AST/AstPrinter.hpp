...
namespace Volt
{
    ...
    namespace Frontend
    {
        ...
        namespace Detail
        {
            ...
            struct IsNodeList<Core::SmallVec<T, N>> : std::true_type
            {
            };
        ...
        private:
            ...
            void PrintVariant( const Variant& Node, int Depth )
            {
                Out << '(' << NodeName( Node );
            ...
            void PrintStruct( std::string_view Name, const Struct& Value, int Depth )
            {
                Out << '(' << Name;
            ...
            template <typename Struct>
            void PrintFieldsOf( const Struct& Value, int Depth )
            {
                Meta::ForEachField( Value, [&]( const char* FieldName, const auto& FieldValue )
                                    {
            ...
            template <typename F>
            void PrintValue( const F& Value, int Depth )
            {
                if constexpr ( std::same_as<F, Symbol> )
                {
                    Out << '\'' << ( Value.IsValid() ? Context.Strings().Resolve( Value ) : std::string_view{} ) << '\'';
                }
                else if constexpr ( std::same_as<F, ExprId> || std::same_as<F, StmtId> || std::same_as<F, DeclId> || std::same_as<F, TypeId> ||
                                    std::same_as<F, ParamId> )
                {
                    if ( Value.IsValid() )
                    {
                        Print( Value, Depth );
