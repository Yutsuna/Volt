...
7. **[FAIT - 2026-07-24] Distinction des Closures Évasives vs Non-évasives (*Escaping Closures*) :**
   - **Statut :** RÉALISÉ & VALIDÉ. Voir Étape D (§VII.2) pour l'implémentation complète (`ScopeTable::Escapes`, `Call`/`DotCall` en position d'argument direct, `ClosureEnvFrame::bEscapes`).

8. **[FAIT - 2026-07-24] Alimentation des `DeclId` (Membres) dans `BindingSite` (portée restreinte) :**
   - **Statut :** RÉALISÉ & VALIDÉ, sur un périmètre volontairement restreint. `BindingSite` (`ScopeTable.hpp`) portait déjà une alternative `Frontend::DeclId` réservée depuis PR #49 (`Feat/Add-scopes`) mais jamais construite — les membres restaient exclusivement résolus par nom-sur-receveur (`TypeStore`/`MemberResolver`), jamais par la chaîne lexicale. `WalkTypeBody` (`ScopeResolver.cpp`) déclare désormais chaque `Field`/`Method` du corps propre d'un type dans le scope `EScopeKind::Type`, en deux passes (déclaration puis descente) puisque la visibilité d'un membre est indépendante de l'ordre textuel, contrairement à un `LocalDecl`. Le cas `InstanceVar` (`@name`) enregistre en plus une liaison best-effort via `Context.Scopes.Resolve`/`BindUse`, mais **sans** rediriger le typage : `MemberType`/`TypeStore` reste la seule autorité, et `ScopeTable::BindingOf` n'est consommé nulle part pour un `InstanceVar` (seul `Identifier` via `FindLocal`/`WriteLocal` le lit) — donc zéro changement de comportement observable, uniquement de la métadonnée structurelle pour un futur outillage (go-to-definition). Volontairement exclu de ce lot : l'unification complète (rediriger `@field`/`self.field` par `ScopeTable`) et tout nouveau diagnostic de redéclaration de membre (absent aujourd'hui de `TypeStore::AddMember`, ajouter cette vérification ici aurait été un changement de comportement, hors du périmètre restreint retenu).
   - **Couverture :** aucune nouvelle golden dédiée — pas de surface externe (dumper/diagnostic) exposant les bindings de `ScopeTable` aujourd'hui, même précédent que `bEscapes` (Étape D, §VII.2). Non-régression complète : 134/134 tests verts, `volt check source/Lib/` toujours 0 erreur (15 fichiers, 26 décls exportées), `volt-build tidy` propre, graphe `graphify` rafraîchi.

9. **[FAIT - 2026-07-24] Égalité des `Enum` (`===` / `Comparable`) câblée :**
   - **Statut :** RÉALISÉ & VALIDÉ. Plutôt que de doter chaque `enum` d'une méthode `===` concrète (ce qui aurait exigé un discriminant matérialisé dans le layout — un `enum` sans `@[Primitive]` reste aujourd'hui un `Aggregate` à zéro champ, voir `TypeBinder::AggregateOf`), l'exemption déjà en place pour les opérateurs natifs sur layout `Primitive`/`Pointer` (`IsBuiltinOpOn`, `MemberResolver.cpp`) a été élargie à `"==="` sur tout nominal `Aggregate` qui déclare au moins un `EnumCase` — même patron que pour les primitives, zéro nouveau champ AST, zéro nouvelle passe. `HasEnumCases` (le prédicat de garde de l'exhaustivité, `ExprInferencer.cpp`) a été promu du namespace anonyme vers `MemberResolver.hpp`/`.cpp` pour être partagé par les deux call sites (`MemberType` et `DeclStmtWalker`'s abstract-conformance check).
   - **Bug latent découvert et corrigé au passage :** `EnumCaseNameOf` (exhaustivité) ne reconnaissait que la forme `Call(Member(...))` — le sucre `.Name` une fois désugaré — jamais un `Member` nu, alors que `TrafficLight::Red` (receveur explicite) parse justement en `Member` nu (`::` est strictement `.` pour `ParsePostfix`, jamais un `Call`). Sans ce second cas, l'exhaustivité aurait rejeté `when Enum::Case` comme non couvrant *tous* les variants même une fois `===` typable. Corrigé par un troisième embranchement dans `EnumCaseNameOf`.
   - **Couverture :** `samples/Sema/EnumExplicitReceiver.vl` (fonction libre `describe(light: TrafficLight) -> String`, `case light when TrafficLight::Red / ...Yellow / ...Green` exhaustif par receveur explicite). 134/134 tests verts, `volt check source/Lib/` toujours 0 erreur (15 fichiers, 26 décls exportées), `volt-build tidy` propre, graphe `graphify` rafraîchi.
...
2. **[FEUILLE DE ROUTE FRONTEND / MIDDLE-END] :**
   ...
   - **[FAIT - 2026-07-24] Étape D : Closures Évasives vs Non-évasives (*Escaping Closures*) :** `ClosureFrame` dimensionnait déjà les environnements de capture mais ne portait aucune information sur leur durée de vie. Ajout de `ScopeTable::SetEscapes` / `Escapes( ScopeId )` (`ScopeTable.hpp`), même patron de table annexe que `ClosureCaptures` (`std::unordered_map<ScopeId, bool>`), avec `true` (évasif) comme valeur par défaut conservatrice en l'absence de preuve. La preuve est structurelle et calculée dans `ScopeResolver.cpp` sans nouvelle passe : `WalkExpr` gagne un paramètre `bDirectCallArg` (non propagé aux enfants — seule la position d'argument elle-même compte, pas ce qu'elle contient), et deux nouveaux embranchements `Call`/`DotCall` marquent leurs `Args`/`BlockArg` comme positions directes. Un littéral `Block`/`Lambda` rencontré dans une position directe (`arr.map do | x | ... end`, `numbers.map( &transform )`, `f( &.+ 10 )`) est non-évasif ; partout ailleurs (`LocalDecl`, `Assign`, `Return`, élément de littéral tableau/hash, champ de struct) il reste évasif par défaut. `ClosureEnvFrame` gagne le champ `bEscapes`, peuplé dans `SynthesizeClosureFrame` depuis `Scopes.Escapes( ClosureScope )` — la donnée que le futur backend consommera pour choisir pile vs tas, au même titre que les champs de taille/alignement déjà calculés (PR #50). Zéro nouveau nœud AST, zéro nouvelle passe (`PassList.inl` inchangé), zéro régression : 134/134 tests verts, `volt check source/Lib/` toujours 0 erreur, `volt-build tidy` propre. `SynthesizeClosureFrame` reste — comme avant ce chantier — un maillon de layout calculé mais non encore consommé par aucune passe : sans backend/codegen (§VII.3, toujours ULTÉRIEUR), il n'existe pas de surface de test golden dédiée à `bEscapes` au-delà de la non-régression du pipeline existant.
   - **[FAIT - 2026-07-24] Étape F : Complétion de la stdlib & Résolution des expressions point-free composées (`FunctionalSpec.vl`) :**
     - **Parsing et Precedence des Pipelines (`|>`) :** Réajustement de la *binding power* de `PipeGreater` et `LessPipe` dans `Pratt.inl` (passée de `18` à `45`, supérieure à l'égalité `40`) afin que `4 |> double == 8` se parse correctement en `(4 |> double) == 8`.
     - **Complétion de `String.vl` & Fonctions Libres :** Implémentation des méthodes `trim`, `downcase`, `upcase` et `prefix` dans `source/Lib/Primitives/String.vl`. Ajout de la fonction libre top-level `def raise( message : String ) -> Void`.
     - **Fallback de Lookup sur Littéraux Non Contraints & Inférence des Lambdas Composées :** Extension de `LookupOn` dans `MemberResolver.cpp` pour retomber dynamiquement sur le nominal revendiqué (`Int32`/`Float64`) sans aucun hardcode de nom de type C++. Implémentation de la propagation préalable des types d'arguments vers `ExpectedClosure` dans `CallType` (`ExprInferencer.cpp`) lors de l'appel direct d'un nœud `Lambda` (tel que généré par `LowerComposition`), permettant à `numbers.map( ( &.+ 10 ) >> ( &.* 2 ) )` et `raw_users.filter( (&.empty?.!) ).map( transform )` de se typer sans annotation explicite.
     - **Couverture & Validation :** `samples/Functional/FunctionalSpec.vl` (76 lignes, 0 erreur, 4 pipelines désucrés). 134/134 tests verts (`volt-build format test`), typage complet de la stdlib (`volt check source/Lib/` : 15 fichiers, 27 décls exportées, 0 erreur), conformité `ZeroHardcode` (0 fuite), `volt-build tidy` propre, et graphe `graphify` mis à jour (`510` nœuds).
   - **[FAIT - 2026-07-24] Étape G : Complétion Primitives & Mixins (`CharLiteral`, `Symbol` & `===`) :**
   ...
   - **[FAIT - 2026-07-24] Étape H : `raise`/`rescue`/`ensure` — revue et corrections post-implémentation :**
     - **Zero-hardcode (`"Exception"`) :** Le compilateur mentionnait littéralement `"Exception"` à trois endroits (désucrage `raise "msg"` dans le parseur, typage de `rescue` dans `ExprInferencer.cpp`). Ajout d'une nouvelle annotation `@[ExceptionRoot]` (même patron que `@[Primitive]`/`@[Literal]`) : `TypeStore::SetExceptionRoot`/`GetExceptionRoot` (liaison à propriétaire unique, refuse la re-liaison), consommée dans `TypeBinder::BindType`, posée sur `class Exception` (`source/Lib/Primitives/Exception.vl`). Le désucrage `raise "msg"` → `Exception.new("msg")` a été **déplacé du parseur vers `ExprInferencer.cpp`** (`RaiseExpr`), seul endroit où `TypeStore` est lié — le parseur reste purement syntaxique.
     - **`raise` nu / re-raise (`.agents/PLAN_RAISE.md` §1) :** Jusqu'ici non implémenté — `Expr.Exception` invalide ne déclenchait ni re-raise ni construction de secours. Ajout de `TypeCheckerContext::RescueVarStack` (empilé/dépilé autour du corps de chaque `RescueClause`) : un `raise` nu à l'intérieur d'un `rescue` re-lève désormais la variable capturée ; en dehors de tout `rescue`, il retombe sur `Exception.new()` via un nouvel helper partagé `MakeExceptionConstructor`.
     - **Scope manquant sur `BeginExpr` :** Contrairement à `If`/`While` (qui ouvrent un scope `Branch` explicite dans `ScopeResolver.cpp`), le corps principal et le bloc `ensure` d'un `begin...end` retombaient dans le parcours réflexif générique **sans** nouveau scope — des locales déclarées dedans fuyaient vers le scope englobant. Ajout d'un embranchement `BeginExpr` dédié dans `WalkExpr` poussant un scope `Branch` séparé pour `Body` et pour `EnsureBody` (les `RescueClauses` gardent leur propre scope, déjà correct).
     - **Arité et paramètres par défaut non-terminaux :** `MinParams` ne comptait que les paramètres sans défaut, sans jamais vérifier qu'un paramètre par défaut n'est pas suivi d'un paramètre obligatoire (`def f(a: Int32 = 1, b: Int32)` type-checkait `f(x)` à tort). Ajout de `SignatureResolver::CheckTrailingDefaults` (`TypeBinder.cpp`), câblée sur les deux sites de résolution de signature (méthode et fonction libre) ; `Diags` — jusqu'ici ignoré (`static_cast<void>`) dans `ResolveUnitSignatures` — est maintenant effectivement transmis.
     - **Duplication du parsing `rescue`/`ensure` :** La boucle `while rescue ... end` + `ensure` optionnel était copiée à l'identique entre l'expression `begin` (`ParseExpr.cpp`) et le rescue implicite de méthode (`ParseDecl.cpp`). Extraction en un seul `Parser::ParseRescueEnsure(BeginExpr&)` partagé par les deux sites.
     - **`super` par comparaison de chaîne :** `super` était désucré en un `Identifier{"super"}` générique, puis re-détecté plus tard par `Text(Expr.Name) == "super"` dans `ExprInferencer.cpp` — fragile et non conforme au style à nœuds typés du reste de l'AST. Ajout d'un nœud dédié `SuperExpr` (même patron que `SelfExpr`, un manifeste `Nodes.inl` + une struct réfléchie, aucun changement de dumper/walker nécessaire).
     - **Couverture & Validation :** `samples/Sema/RaiseDivergence.vl`, `samples/Syntax/ControlFlow/BeginRescue.vl`, `samples/Syntax/ControlFlow/UntilLoop.vl` (créés lors du chantier `raise`/`rescue` initial, avant cette passe de revue). Régénération de la totalité des fixtures `tests/golden/` via la cible `golden-update` (rattrapage d'un désalignement antérieur au chantier de revue : plusieurs échantillons utilisant la macro `assert` reflétaient encore un `raise` désucré en simple `Call` plutôt qu'en `RaiseExpr`). **143/143 tests verts** (`volt-build format test`), `volt check source/Lib/` toujours à 0 erreur (17 fichiers, 36 décls exportées), conformité `ZeroHardcode` (0 fuite, `grep -RnE '"Exception"'` sur `Frontend`/`Sema` ne retourne plus rien hors annotation/commentaire). `volt-build tidy` non exécuté dans cette passe (laissé à l'utilisateur).
