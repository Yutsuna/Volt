...
namespace Driver
{
    ...
    CircuitGraph::NodeIndex CircuitGraph::AddModule ( std::string Name )
    {
    ...
    void CircuitGraph::AddLink ( const std::string &From, const std::string &To )
    {
        const NodeIndex FromNode = AddModule( From );
        const NodeIndex ToNode   = AddModule( To );

        std::vector<NodeIndex> &Out = Edges[FromNode];
        if ( std::find( Out.begin(), Out.end(), ToNode ) == Out.end() )
        {
            Out.push_back( ToNode );
        }
    }

    CircuitGraph::NodeIndex CircuitGraph::Find ( const std::string &Name ) const
    {
    ...
    bool CircuitGraph::TopoOrder ( std::vector<NodeIndex> &Order ) const
    {
        Order.clear();
    ...
    std::vector<CircuitGraph::NodeIndex> CircuitGraph::FindCycle () const
    {
        const std::size_t Count = Names.size();
