...
- **Backend Is Machine-Only.** All 3 backends (LLVM, WASM, VM) know only `i8..i64`, `u8..u64`, `f32`/`f64`, pointers, references, (de)referencing — never a Volt construct's structure or protocol, even without a Volt type name spelled in C++. Anything else (a literal's construction protocol, an allocation) is resolved and lowered by the middle-end first. See [`rules/backend-machine-only.md`](rules/backend-machine-only.md).
...
- **25-Node Core AST Contract.** No sugar survives `TypeChecker` (nine kinds lowered by an `EPassKind::Lowering` pass, `ArrayLit`/`HashLit` each by a post-walk sweep inside `TypeChecker` itself), every value expression is typed (outright in concrete code, or after substitution inside a generic body), and `AstInvariant` (Order 40) enforces both on every build. What is refused loudly upstream rather than delegated to a backend is specified in [`rules/core-ast.md`](rules/core-ast.md).
...
- **Build Commands & Concurrency Strategy.** `ninja -C build` for Debug (g3), `ninja -C build-asan` for ASAN+Debug. **NE JAMAIS LANCER PLUSIEURS BUILDS EN MÊME TEMPS (JAMAIS).** **NE JAMAIS UTILISER MANAGE_TASK OU SCHEDULE.** See [`rules/build-performance.md`](rules/build-performance.md).
