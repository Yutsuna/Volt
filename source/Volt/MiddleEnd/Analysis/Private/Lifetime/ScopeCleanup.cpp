...
#include "Volt/MiddleEnd/Analysis/Lifetime/ExitPaths.hpp"
...
namespace
{
    ...
    using namespace Volt::MiddleEnd::Analysis::Lifetime;
    ...
    struct FinalizeCandidate
    {
        ...
        const MiddleEnd::Resolver::Binding *DirectBinding = nullptr;
    };
    ...
    [[nodiscard]] bool IsFinalizeCandidateType ( TypeCheckerContext &Context, MiddleEnd::TypeSystem::SemaTypeId LocalType )
    {
        return MiddleEnd::Analysis::Raii::IsFinalizeCandidateType( Context.Ctx.Types, Context.Ctx.Values, LocalType );
    }
    ...
    [[nodiscard]] bool EscapesAsCallArgument ( TypeCheckerContext &Context, const MiddleEnd::Resolver::Binding &CandidateBinding )
    {
        ...
        for ( std::size_t Index = 0; Index < Ast.ExprCount(); ++Index )
        {
            ...
            for ( std::size_t Arg = 0; Arg < CallNode->Args.Size(); ++Arg )
            {
                ...
                if ( bConstructs or Callee == nullptr or MiddleEnd::Analysis::Raii::ParameterEscapes( *Callee, Arg ) )
                {
                    return true;
                }
            }
    ...
    CollectCandidates ( TypeCheckerContext &Context, MiddleEnd::Resolver::ScopeId MethodScope, const Frontend::StmtList &Body )
    {
        ...
        const MiddleEnd::Resolver::Scope &ScopeRef = Context.Ctx.Scopes.Get( MethodScope );
        for ( const Volt::Core::Symbol Name : ScopeRef.Order )
        {
            ...
            const MiddleEnd::Resolver::Binding &Entry = BindingIt->second;
            ...
            const MiddleEnd::TypeSystem::SemaTypeId LocalType = Context.Ctx.Values.SiteType( Entry.Site );
    ...
    [[nodiscard]] Frontend::ExprId BuildFinalizeCall ( TypeCheckerContext &Context,
                                                       ...
                                                       MiddleEnd::TypeSystem::SemaTypeId LocalType,
                                                       MiddleEnd::Resolver::ScopeId MethodScope )
    {
        ...
        auto MakeReadId = [&] () -> Frontend::ExprId
        {
            ...
            else if ( const MiddleEnd::Resolver::Binding *Bound = Context.Ctx.Scopes.Resolve( MethodScope, Candidate.Name ) )
            {
    ...
    BuildRescueCandidate ( TypeCheckerContext &Context, const Frontend::RescueClause &Clause, Frontend::StmtId RescueId )
    {
        ...
        const MiddleEnd::Resolver::ScopeId RescueScope = Context.Ctx.Scopes.ScopeOf( RescueId );
        if ( not RescueScope.IsValid() )
        {
            return std::nullopt;
        }
        const MiddleEnd::TypeSystem::SemaTypeId LocalType =
            Context.Ctx.Values.SiteType( MiddleEnd::Resolver::BindingSite{ RescueId } );
        if ( not IsFinalizeCandidateType( Context, LocalType ) )
        {
            return std::nullopt;
        }
        const MiddleEnd::Resolver::Scope &RescueScopeRef = Context.Ctx.Scopes.Get( RescueScope );
        const auto BindingIt                             = RescueScopeRef.Bindings.find( Clause.VarName );
        if ( BindingIt == RescueScopeRef.Bindings.end() )
        {
            return std::nullopt;
        }
        return FinalizeCandidate{ .Site          = MiddleEnd::Resolver::BindingSite{ RescueId },
                                  .Name          = Clause.VarName,
                                  .BodyIndex     = 0,
                                  .DirectBinding = &BindingIt->second };
    }
                                                    ...
                                                    Frontend::StmtList *DeferredExit = nullptr )
    {
        ...
        for ( std::size_t BodyPos = 0; BodyPos < Body.Size(); ++BodyPos )
        {
            ...
            if ( bIsReturn or bIsLoopExit )
            {
                ...
                for ( const FinalizeCandidate &Candidate : ToFinalize )
                {
                    ...
                    const MiddleEnd::TypeSystem::SemaTypeId LocalType = Context.Ctx.Values.SiteType( Candidate.Site );
        ...
        MiddleEnd::TypeSystem::SemaTypeId TailType;
    ...
    // ordinary code.
    [[nodiscard]] bool ScopeHasAnyFinalizeCandidate ( TypeCheckerContext &Context, MiddleEnd::Resolver::ScopeId MethodScope )
    {
        if ( not MethodScope.IsValid() )
        {
            return false;
    ...
    [[nodiscard]] WriteSet CollectWrites ( TypeCheckerContext &Context, const MiddleEnd::Resolver::Binding &Local )
    {
        const Frontend::AstContext &Ast = Context.Ctx.Ast;
    ...
    [[nodiscard]] std::uint32_t DeclaringWriteOf ( const MiddleEnd::Resolver::Binding &Local )
    {
    ...
    void EmitDropBeforeStore ( TypeCheckerContext &Context,
                               const MiddleEnd::Resolver::Binding &Local,
                               const Volt::Core::Symbol LocalName,
                               const MiddleEnd::TypeSystem::SemaTypeId LocalType,
                               const Frontend::ExprId AssignId )
    {
        Frontend::AstContext &Ast         = Context.Ctx.Ast;
        const Volt::Core::SourceRange Loc = std::get<Frontend::Assign>( Ast.Expr( AssignId ) ).Loc;
        ...
        const Volt::Core::Symbol OldName = Ast.MakeUniqueSymbol( "__old" );
        ...
        Context.Ctx.Scopes.Declare( RegionScope, OldName, MiddleEnd::Resolver::BindingSite{ OldTarget } );
        const MiddleEnd::Resolver::Binding *OldBound = Context.Ctx.Scopes.Resolve( RegionScope, OldName );
        if ( OldBound != nullptr )
        {
            Context.Ctx.Scopes.BindUse( OldTarget, *OldBound, false );
        }
        Context.Ctx.Values.SetExprType( OldTarget, LocalType );
        Context.Ctx.Values.SetSiteType( MiddleEnd::Resolver::BindingSite{ OldTarget }, LocalType );
        Context.LocalTypes[MiddleEnd::Resolver::BindingSite{ OldTarget }] = LocalType;
...
bool RunDropOnReassign ( TypeCheckerContext &Context, const MiddleEnd::Resolver::ScopeId Scope, const Frontend::StmtList &Body )
{
    ...
    bool bChanged                              = false;
    const MiddleEnd::Resolver::Scope &ScopeRef = Context.Ctx.Scopes.Get( Scope );
    ...
    const std::vector<Volt::Core::Symbol> Names{ ScopeRef.Order.begin(), ScopeRef.Order.end() };

    for ( const Volt::Core::Symbol Name : Names )
    {
        const MiddleEnd::Resolver::Scope &Live = Context.Ctx.Scopes.Get( Scope );
        const auto BindingIt                   = Live.Bindings.find( Name );
        if ( BindingIt == Live.Bindings.end() )
        {
            continue;
        }
        const MiddleEnd::Resolver::Binding &Local         = BindingIt->second;
        const MiddleEnd::TypeSystem::SemaTypeId LocalType = Context.Ctx.Values.SiteType( Local.Site );
        if ( not IsFinalizeCandidateType( Context, LocalType ) )
        {
            continue;
        }
