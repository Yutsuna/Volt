...
[[nodiscard]] bool EnumHasAnyPayload ( const Volt::Sema::TypeStore &Store, Volt::Sema::NominalId Base )
{
    if ( not Base.IsValid() )
    {
        return false;
    }
    return std::ranges::any_of( Store.Type( Base ).Members, [] ( const Sema::Member &Entry )
                                { return Entry.Kind == Sema::EMemberKind::EnumCase and Entry.Params.Size() > 0; } );
}
...
void LowerEnumCaseConstant ( Volt::Sema::TypeCheckerPass::TypeCheckerContext &Context, Frontend::ExprId Id, std::int64_t Ordinal )
{
    Frontend::AstContext &Ast   = Context.Ctx.Ast;
    const Core::SourceRange Loc = Frontend::LocOf( Ast.Expr( Id ) );
    Ast.Expr( Id ) =
...
void LowerEnumCaseConstruction ( Volt::Sema::TypeCheckerPass::TypeCheckerContext &Context,
                                 Frontend::ExprId Id,
                                 Frontend::ExprList Args,
                                 std::string_view CaseName,
                                 std::int64_t Ordinal,
                                 Sema::SemaTypeId ResultType )
{
    ...
    const Sema::NominalId Base      = Context.Ctx.Values.Get( ResultType ).Base;
    ...
    Sema::ScopeId CurrentScope = Context.Ctx.Scopes.ScopeOfExpr( Id );
    if ( not CurrentScope.IsValid() )
    {
        CurrentScope = Sema::ScopeId{ 0 };
    }
...
void BindPatternUseStmt ( Volt::Sema::TypeCheckerPass::TypeCheckerContext &Context,
                          Frontend::StmtId Id,
                          Frontend::Symbol Name,
                          const Sema::Binding &NewBinding,
...
void BindPatternUse ( Volt::Sema::TypeCheckerPass::TypeCheckerContext &Context,
                      Frontend::ExprId Id,
                      Frontend::Symbol Name,
                      const Sema::Binding &NewBinding,
                      Sema::SemaTypeId FieldType,
...
void Volt::Sema::TypeCheckerPass::LowerEnumCases ( TypeCheckerContext &Context )
{
    ...
    for ( std::size_t Index = 0; Index < OriginalCount; ++Index )
    {
        ...
        if ( std::holds_alternative<Frontend::Member>( Node ) )
        {
            ...
            {
                const Sema::NominalId Base = Context.Ctx.Values.Has( Entry->second.Receiver )
...
void Volt::Sema::TypeCheckerPass::LowerEnumPatterns ( TypeCheckerContext &Context )
{
    // Bound before the first Add() — every node this rewrite creates lands
    ...
    for ( std::size_t Index = 0; Index < OriginalCount; ++Index )
    {
        ...
        for ( const Frontend::StmtId ClauseId : Clauses )
        {
            ...
            for ( const Frontend::ExprId PatternId : Clause.Patterns )
            {
                ...
                const Sema::NominalId Base     = ScrutineeType.IsValid() and Context.Ctx.Values.Has( ScrutineeType )
                                                     ? Context.Ctx.Values.Get( ScrutineeType ).Base
                                                     : Sema::NominalId{};
                const Sema::Member *CaseMember =
                ...
                const Sema::SigTypeId PayloadSigType = bWantsPayload ? CaseMember->Params[0] : Sema::SigTypeId{};
                ...
                const Core::SmallVec<SemaTypeId, 2> ScrutineeArgs = Context.Ctx.Values.Get( ScrutineeType ).Args;
                ...
                // against below.
                Sema::ScopeId CurrentScope = Context.Ctx.Scopes.ScopeOfExpr( BindTarget );
                if ( not CurrentScope.IsValid() )
                {
                    CurrentScope = Sema::ScopeId{ 0 };
                }
                Context.Ctx.Scopes.Declare( CurrentScope, BindingNameSym, BindTarget );
                const Sema::Binding *Bound = Context.Ctx.Scopes.Resolve( CurrentScope, BindingNameSym );
                if ( Bound != nullptr )
                {
