...
namespace Backend
{
    ...
    namespace Llvm
    {
        ...
        public:
            ...
            //
            // `llvm::Value *`, not `llvm::GlobalVariable *`, and taking the
            // builder to emit into: under ETlsAccess::Accessor the address is
            // the result of a *call*, so it belongs to the block being written
            // and cannot be cached for the module the way a global can. Every
            // caller already has its builder in hand; naming it is what keeps
            // the two modes from differing anywhere else.
            [[nodiscard]] llvm::Value *ExceptionValueSlot ( llvm::IRBuilderBase &Into );
            [[nodiscard]] llvm::Value *ExceptionTagSlot ( llvm::IRBuilderBase &Into );
            ...
            [[nodiscard]] llvm::Value *BreakFlagSlot ( llvm::IRBuilderBase &Into );
            ...
            [[nodiscard]] llvm::Value *ExceptionStorageSlot ( llvm::IRBuilderBase &Into );

            // Measures on demand: a program with no `raise` anywhere never
            // creates the buffer, but a JIT still has to size the one it hands
            // back through the accessor table.
            [[nodiscard]] std::size_t StorageSize ()
            {
                MeasureStorage();
        ...
        private:
            ...
            // One slot's address under ETlsAccess::Accessor: call the accessor,
            // then load entry `Index` of the table it hands back.
            // Fills ExcStorageSize / ExcStorageAlign. Idempotent.
            void MeasureStorage ();

            [[nodiscard]] llvm::Value *SlotThroughAccessor ( llvm::IRBuilderBase &Into, std::size_t Index, const char *Name );
