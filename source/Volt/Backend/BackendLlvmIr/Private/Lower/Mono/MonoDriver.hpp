...
// empty rather than running once. The loop itself, and the member lookup, are
// BackendCore's (MonoQueue): both are target-neutral, and what this service
// adds is EmitMonomorphizedBody.
...
namespace Backend
{
    ...
    namespace Llvm
    {
        ...
        public:
            ...
            // MonoQueue::Lookup, with the build already in hand: the member a
            // request names, plus the UnitView that declares it (its
            // Ast/Scopes are what ReinstantiateBody re-types the body against).
            [[nodiscard]] const MiddleEnd::TypeSystem::Member *LookupMonoMember ( const MonoRequest &Request,
                                                                                  const UnitView **OutUnit ) const
            {
                return MonoQueue::Lookup( *Services->Build, Request, OutUnit );
            }
