...
void Volt::Frontend::Parser::ParseComponentFile ()
{
    ParseFile();
}

Volt::Frontend::DeclId Volt::Frontend::Parser::ParseComponent ()
{
...
bool Volt::Frontend::Parser::AtJsxStart () const
{
...
Volt::Frontend::ExprId Volt::Frontend::Parser::ParseJsxElement ()
{
    const std::uint32_t Begin = Here();
    ...
    if ( Accept( TokenKind::Gt ) )
    {
        ...
        return MakeExpr( Fragment, RangeSince( Begin ) );
    }
    ...
    // Attributes.
    while ( not AtEnd() and not Check( TokenKind::Gt ) and not Check( TokenKind::Slash ) )
    {
        SkipNewlines();
        ...
        else
        {
            ...
            BoolLiteral True;
            True.Value = true;
            Value      = MakeExpr( True, RangeSince( Begin ) );
        }
    ...
    if ( Accept( TokenKind::Slash ) )
    {
        ...
        return MakeExpr( Element, RangeSince( Begin ) );
    }
...
void Volt::Frontend::Parser::ParseJsxChildren ( ExprList &Children, std::string_view CloseTag )
{
    for ( ;; )
    {
        if ( AtEnd() )
        ...
        while ( not AtEnd() and not Check( TokenKind::Lt ) and not Check( TokenKind::LBrace ) and not Check( TokenKind::KwEnd ) )
        {
        ...
        if ( TextEnd > TextBegin and TextEnd <= Source.size() )
        {
            ...
            if ( !Slice.empty() )
            {
                ...
                Children.PushBack(
                    MakeExpr( Text, Core::SourceRange{ .File = Context.FileId(), .Begin = TextBegin, .End = TextEnd } ) );
            }
        }
