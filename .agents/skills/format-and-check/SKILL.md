...
description: The finishing checklist for any Volt change — clang-format, a clean -Werror build, clang-tidy, the ctest suites, and a graphify refresh. Use before calling a compiler change done.
---

# Format & check

Run these before finishing a change. Any failure means the change is not done.
Every step goes through `volt-build` (see the `build` skill — never raw
cmake/ninja/ctest); the tooling targets are parallel and per-file cached, so
re-runs only touch what changed.

1. **Format**:
   ```sh
   volt-build format              # repo .clang-format: Allman, SpacesInParens, col 170
   ```
   Covers `.cpp` + `.hpp` + `.inl`; the manifests are protected by their
...
3. **Lint** (respect `.clang-tidy`): resolve or justify every diagnostic on
   ...
   reaches `Meta/Reflect.hpp` are skipped automatically (clang cannot parse
   P2996 until LLVM ships it) — no manual `sed`, no justification needed for
   those.
