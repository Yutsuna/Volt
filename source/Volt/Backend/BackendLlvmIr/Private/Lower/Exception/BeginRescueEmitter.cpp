...
Volt::Backend::Llvm::ExceptionLowering::EmitBegin ( BodyEmitter &Emitter, Frontend::ExprId Id, const Frontend::BeginExpr &Node )
{
    ...
    llvm::Value *Tag    = Builder.CreateLoad( llvm::Type::getInt32Ty( Context ), ExceptionTagSlot( Builder ), "exc.tag" );
    llvm::Value *ExcPtr = Builder.CreateLoad( llvm::PointerType::get( Context, 0 ), ExceptionValueSlot( Builder ), "exc.value" );
    ...
    for ( const Frontend::StmtId ClauseId : Node.RescueClauses )
    {
        ...
        static_cast<void>( Builder.CreateStore( llvm::Constant::getNullValue( llvm::PointerType::get( Context, 0 ) ),
                                                ExceptionValueSlot( Builder ) ) );
    ...
    // saved.
    llvm::Value *SavedTag = Builder.CreateLoad( llvm::Type::getInt32Ty( Context ), ExceptionTagSlot( Builder ), "exc.tag.saved" );
    llvm::Value *SavedExc =
        Builder.CreateLoad( llvm::PointerType::get( Context, 0 ), ExceptionValueSlot( Builder ), "exc.value.saved" );
    llvm::Value *SavedBrk = Builder.CreateLoad( Builder.getInt1Ty(), BreakFlagSlot( Builder ), "brk.saved" );
    static_cast<void>( Builder.CreateStore(
        llvm::ConstantInt::get( llvm::Type::getInt32Ty( Context ), MiddleEnd::TypeSystem::NominalId::InvalidValue ),
        ExceptionTagSlot( Builder ) ) );
    static_cast<void>( Builder.CreateStore( llvm::Constant::getNullValue( llvm::PointerType::get( Context, 0 ) ),
                                            ExceptionValueSlot( Builder ) ) );
    static_cast<void>( Builder.CreateStore( Builder.getFalse(), BreakFlagSlot( Builder ) ) );

    Emitter.EmitStmts( Node.EnsureBody, false );

    if ( not Emitter.Terminated() )
    {
        static_cast<void>( Builder.CreateStore( SavedTag, ExceptionTagSlot( Builder ) ) );
        static_cast<void>( Builder.CreateStore( SavedExc, ExceptionValueSlot( Builder ) ) );
        static_cast<void>( Builder.CreateStore( SavedBrk, BreakFlagSlot( Builder ) ) );
        ...
        // past it.
        llvm::Value *StillTag = Builder.CreateLoad( llvm::Type::getInt32Ty( Context ), ExceptionTagSlot( Builder ), "exc.tag" );
        llvm::Value *StillExcPending = Builder.CreateICmpNE(
            ...
            "exc.still_pending" );
        llvm::Value *StillBrk     = Builder.CreateLoad( Builder.getInt1Ty(), BreakFlagSlot( Builder ), "brk.still_pending" );
