...
namespace Sema
{
    ...
    namespace
    {
        ...
        {
            return std::visit( Meta::Overloaded{ [] ( std::monostate ) { return std::string{}; }, [] ( bool B )
                                                 { return std::string( B ? "true" : "false" ); }, [] ( std::int64_t N )
...
#include "Volt/Sema/MacroOps.inl"
        ...
        {
            while ( !Text.empty() and
                    ( Text.front() == ' ' or Text.front() == '\t' or Text.front() == '\n' or Text.front() == '\r' ) )
            {
                Text.remove_prefix( 1 );
            }
            while ( !Text.empty() and
                    ( Text.back() == ' ' or Text.back() == '\t' or Text.back() == '\n' or Text.back() == '\r' ) )
            {
        ...
        public:
            ...
            explicit MacroExpander ( PassContext &InContext ) : Context( InContext ), Ast( InContext.Ast )
            {
            }
        ...
        private:
            ...
            void CollectDefs ()
            {
                ...
                for ( std::size_t Index = 0; Index < Count; ++Index )
                {
                    ...
                    if ( KindOf( Ast.Decl( Id ) ) == DeclKind::MacroDef )
                    {
                        Defs.insert_or_assign( Ast.Text( std::get<MacroDef>( Ast.Decl( Id ) ).Name ), Id );
                    }
                }
            }
            ...
            std::size_t ExpandRound ()
            {
                ...
                for ( std::size_t Index = 0; Index < Count; ++Index )
                {
                    ...
                    case DeclKind::Module:
                        Expanded += ExpandIn<Module>( Id );
                        break;
                    case DeclKind::Class:
                        Expanded += ExpandIn<Class>( Id );
                        break;
                    case DeclKind::Struct:
                        Expanded += ExpandIn<Struct>( Id );
                        break;
                    case DeclKind::Mixin:
            ...
            template <typename NodeType> [[nodiscard]] std::size_t ExpandIn ( DeclId Id )
            {
                NodeType Node = std::get<NodeType>( Ast.Decl( Id ) );

                DeclList NewBody;
                std::size_t Expanded = 0;
                for ( const DeclId Child : Node.Body )
                {
                    if ( !Child.IsValid() or KindOf( Ast.Decl( Child ) ) != DeclKind::MacroInvoke )
                    {
                        NewBody.PushBack( Child );
                        continue;
                    }

                    const MacroInvoke Invoke = std::get<MacroInvoke>( Ast.Decl( Child ) );
                    for ( const DeclId Generated : ExpandInvoke( Invoke ) )
                    {
                ...
                if ( Expanded > 0 )
                {
                    ...
                    Ast.Decl( Id ) = DeclNode{ std::move( Node ) };
                }
                return Expanded;
            }

            [[nodiscard]] DeclList ExpandInvoke ( const MacroInvoke &Invoke )
            {
                const auto Found = Defs.find( Ast.Text( Invoke.Name ) );
                if ( Found == Defs.end() )
                {
                    Context.Diags.Error( Invoke.Loc,
                                         "no macro named '" + std::string( Ast.Text( Invoke.Name ) ) + "' in this file" );
                    return {};
                }
                const MacroDef Def = std::get<MacroDef>( Ast.Decl( Found->second ) );
            ...
            void BindArguments ( const MacroInvoke &Invoke, const MacroDef &Def )
            {
                ...
                for ( std::size_t Index = 0; Index < Invoke.Args.Size(); ++Index )
                {
                    ...
                    {
                        Context.Diags.Error( Invoke.Loc,
                                             "too many arguments for macro '" + std::string( Ast.Text( Def.Name ) ) + "'" );
                    }
                }
            }
            ...
            [[nodiscard]] MacroValue Eval ( ExprId Id )
            {
                ...
                return std::visit(
                    Meta::Overloaded{ [&] ( const Frontend::IntLiteral &Node )
                                      {
                                          std::int64_t Value          = 0;
                                      ...
                                      [&] ( const Frontend::Identifier &Node )
                                      {
                                          ...
                                          if ( Found == Env.end() )
                                          {
                                              Context.Diags.Error( Loc, "unknown name '" + std::string( Ast.Text( Node.Name ) ) +
                                                                            "' in macro expression" );
                                              return MacroValue{};
                                          }
                                          return Found->second;
                                      },
                                      [&] ( const Frontend::Member &Node )
                                      { return ApplyOp( Eval( Node.Object ), Ast.Text( Node.Name ), Loc ); },
                                      ...
                                      [&] ( const Frontend::Unary &Node )
                                      {
                                          ...
                                          if ( Node.Op == TokenKind::Minus and
                                               std::holds_alternative<std::int64_t>( Operand.Data ) )
                                          {
            ...
            void Emit ( const SegmentList &Segments, std::string &Out )
            {
                for ( const MacroSegment &Segment : Segments )
                {
                    std::visit(
                        Meta::Overloaded{
                            [&] ( const TextSegment &Node ) { Out += Node.Text; }, [&] ( const SpliceSegment &Node )
            ...
            PassContext &Context;
            AstContext &Ast;
            std::unordered_map<std::string_view, DeclId> Defs;
